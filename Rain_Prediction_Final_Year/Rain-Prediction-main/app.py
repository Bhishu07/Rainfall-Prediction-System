...
model_fe = pickle.load(open("./models/catf.pkl", "rb"))
print("Model Loaded")
...
@app.route("/predict_fe",methods=['GET', 'POST'])
@cross_origin()
def predict_fe():
	if request.method == "POST":
		# DATE
		...
		input_lst = [ rainfall , sunshine ,
					 ...
					 rainToday , month , day]
		pred = model_fe.predict(input_lst)
		output = pred
		if output == 0:
			return render_template("after_sunny.html")
		else:
			return render_template("after_rainy.html")
	return render_template("predictor_fe.html")
